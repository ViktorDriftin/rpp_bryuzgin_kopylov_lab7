...
bot_token = os.getenv('BOT_TOKEN') # Получение токена из переменных окружения
...
async def save_converted(message: types.Message, state: FSMContext):
    ...
    if YES in check:  # проверяет хотим ли добавить ещё валюты для конвертации. Если "Да" то переходим на состояние
        ...
        saved_state_global["baseCurrency"] = str(cur["baseCurrency"])  # записываем в словарь название основной влюты по ключу
        saved_state_global["rates"] = cur["rates"]   # записываем в словарь данные валют для конвертации по ключу
        requests.post("http://localhost:10660/load", json=saved_state_global)   # отправляем запрос с данными в микросервис
...
async def process_convert2(message: types.Message, state: FSMContext):
    ...
    cur = await state.get_data()  # записывает в переменню данные из состояния
    saved_state_global["baseCurrency"] = str(cur["baseCurrency"])  # записываем в словарь название основной влюты по ключу
    saved_state_global["convertedCurrency"] = str(cur["convertedCurrency"])  # записываем в словарь название валюты для конвертации
    # по ключу
    saved_state_global["sum"] = float(num)  # записываем в словарь сумму для конвертации
    result = requests.get("http://localhost:10606/convert", params=saved_state_global)   # отправляем запрос с данными в микросервис
